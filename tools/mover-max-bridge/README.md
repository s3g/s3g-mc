...
This optional Max bridge reads exported Mover JSON files and plays the stored
...
Open `Mover AmbiMonitor Bridge.maxpat`, drop a Mover JSON export onto the
...
Disabled or muted Mover sources are skipped in ICST mode.

When a JSON file loads, the patch reports the file duration in the visible
`total seconds` number box. This value comes from the Mover JSON `duration`
field. Current Mover exports set that duration from the full A-H motion scene
...
Mover export becomes a 48 second forward/backward cycle.

The bridge reads the exported `automation` arrays from Mover JSON rather than
