...
Current package snapshot: the browser exposes 116 user-facing tools/controllers, plus the Package Browser. Of those, 48 are Python/NumPy-backed offline processes, 25 load, control, or render included JSFX, and 43 are native Lua/ReaImGui REAPER actions. The repository also ships 23 underlying JSFX engine/effect files. The Lua folder includes helper libraries and internal dialogs that are not shown as Package Browser entries.
