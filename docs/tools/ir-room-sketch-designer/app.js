...
function settings() {
  ...
  const directionCount = directionSetDirections(directionSet).length;
  return {
    room_x: roomX,
    room_y: roomY,
    room_z: roomZ,
...
function roomPoints(s) {
  const listener = { x: s.room_x * 0.5, y: s.room_y * 0.5, z: s.room_z * 0.5 };
  const unit = unitFromAed(s.source_azimuth, s.source_elevation);
...
function reflectionEvents(s) {
  const { listener, source } = roomPoints(s);
  const images = [
...
function drawRoomView(s) {
  ...
  drawPoint(px(listenerPoint), py(listenerPoint), 7, "#d7d7d7", "L");
  drawPoint(px(sourcePoint), py(sourcePoint), 8, "#5aa8c7", "S");
  ctx.fillStyle = "#9a9a9a";
  ctx.font = "11px Menlo, monospace";
  ctx.fillText(`${state.view.toUpperCase()} ${s.room_x.toFixed(1)} x ${s.room_y.toFixed(1)} x ${s.room_z.toFixed(1)} m`, 12, 20);
}
...
function drawDirections(s) {
  const dirs = directionSetDirections(s.direction_set);
  ...
  dirs.forEach((dir, index) => {
    ...
    const z = unit.z;
    ctx.globalAlpha = 0.45 + Math.max(0, z) * 0.45;
    ctx.fillStyle = "#5aa8c7";
    ctx.beginPath();
    ctx.arc(x, y, 5 + Math.max(0, z) * 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#d7d7d7";
    ctx.font = "10px Menlo, monospace";
    ctx.fillText(String(index + 1), x + 8, y + 3);
  });
  ctx.globalAlpha = 1;
  ctx.fillStyle = "#9a9a9a";
  ctx.fillText(`${dirs.length} directions / ${s.channels_per_ir}ch per IR / ${s.stacked_channels}ch stacked`, 12, 20);
}
...
function exportObject(s = settings()) {
  return {
    ...
    directions: directionSetDirections(s.direction_set).map((d, i) => ({ index: i + 1, azimuth: d[0], elevation: d[1] })),
