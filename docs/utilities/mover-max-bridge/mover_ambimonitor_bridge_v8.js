...
function setduration(v) {
  ...
  if (mover) mover.duration = duration;
...
function meta() {
  ...
  outlet(2, ["point_rate", Number(mover.point_rate || 0)]);
  outlet(2, ["groups", (mover.banks || []).length]);
  outlet(2, ["sources", sourceCount()]);
  outlet(2, ["order", Number(mover.order || 3)]);
}
...
function sourceCount() {
  if (!mover || !mover.banks) return 0;
  var count = 0;
  for (var i = 0; i < mover.banks.length; i += 1) {
    count += (mover.banks[i].automation || []).length;
  }
  return count;
}
