...
local function loop()ImGui.SetNextWindowSize(ctx,520,430,ImGui.Cond_Appearing);local vis;vis,open=ImGui.Begin(ctx,"Chaotic Resonant EQ",open);if vis then ImGui.Text(ctx,"Source: "..entry.name.." ("..entry.channels.." ch)");local changed;changed,s.bands=ImGui.SliderInt(ctx,"Bands",math.floor(s.bands),2,48);changed,s.low_freq=ImGui.SliderDouble(ctx,"Low frequency",s.low_freq,20,1000,"%.1f");changed,s.high_freq=ImGui.SliderDouble(ctx,"High frequency",s.high_freq,400,18000,"%.1f");changed,s.q=ImGui.SliderDouble(ctx,"Resonance Q",s.q,1,120,"%.1f");changed,s.feedback=ImGui.SliderDouble(ctx,"Feedback",s.feedback,0,0.92,"%.2f");changed,s.chaos=ImGui.SliderDouble(ctx,"Chaotic detune",s.chaos,0,1,"%.2f");changed,s.wet=ImGui.SliderDouble(ctx,"Wet mix",s.wet,0,1,"%.2f");changed,s.drive=ImGui.SliderDouble(ctx,"Drive",s.drive,0.2,8,"%.2f");changed,s.normalize=ImGui.Checkbox(ctx,"Peak normalize",s.normalize);if s.normalize then changed,s.normalize_db=ImGui.SliderDouble(ctx,"Normalize dB",s.normalize_db,-36,0,"%.1f")end;changed,s.seed=ImGui.InputInt(ctx,"Seed",math.floor(s.seed));ImGui.Text(ctx,"Feedback is soft-limited in the offline backend.");if ImGui.Button(ctx,"Render",96,28)then go=true end;ImGui.SameLine(ctx);if ImGui.Button(ctx,"Cancel",96,28)then open=false end;ImGui.End(ctx)end;persist();if go then open=false;render();return end;if open then reaper.defer(loop)end end
reaper.defer(loop)
