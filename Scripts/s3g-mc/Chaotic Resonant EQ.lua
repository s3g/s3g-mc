...
local function loop()ImGui.SetNextWindowSize(ctx,520,520,ImGui.Cond_Appearing);local vis;vis,open=ImGui.Begin(ctx,"Chaotic Resonant EQ",open);if vis then theme.muted(ImGui,ctx,"Source: "..entry.name.." ("..entry.channels.." ch)");local changed;local sx,sy,sh,stack=sol_ui.begin_section(ImGui,ctx,"Resonators",248);changed,s.bands=sol_ui.draw_slider_int(ImGui,ctx,"Bands",math.floor(s.bands),2,48);changed,s.low_freq=sol_ui.draw_slider(ImGui,ctx,"Low frequency",s.low_freq,20,1000,"%.1f",false);changed,s.high_freq=sol_ui.draw_slider(ImGui,ctx,"High frequency",s.high_freq,400,18000,"%.1f",false);changed,s.q=sol_ui.draw_slider(ImGui,ctx,"Resonance Q",s.q,1,120,"%.1f",false);changed,s.feedback=sol_ui.draw_slider(ImGui,ctx,"Feedback",s.feedback,0,0.92,"%.2f",false);changed,s.chaos=sol_ui.draw_slider(ImGui,ctx,"Chaotic detune",s.chaos,0,1,"%.2f",false);changed,s.wet=sol_ui.draw_slider(ImGui,ctx,"Wet mix",s.wet,0,1,"%.2f",false);changed,s.drive=sol_ui.draw_slider(ImGui,ctx,"Drive",s.drive,0.2,8,"%.2f",false);sol_ui.finish_section(ImGui,ctx,sx,sy,sh,stack);sx,sy,sh,stack=sol_ui.begin_section(ImGui,ctx,"Output",s.normalize and 123 or 98);changed,s.normalize=sol_ui.draw_checkbox(ImGui,ctx,"Peak normalize",s.normalize);if s.normalize then changed,s.normalize_db=sol_ui.draw_slider(ImGui,ctx,"Normalize dB",s.normalize_db,-36,0,"%.1f",false)end;changed,s.seed=sol_ui.draw_input_int(ImGui,ctx,"Seed",s.seed);sol_ui.finish_section(ImGui,ctx,sx,sy,sh,stack);theme.muted(ImGui,ctx,"Feedback is soft-limited in the offline backend.");if ImGui.Button(ctx,"RENDER",96,28)then go=true end;ImGui.SameLine(ctx);if ImGui.Button(ctx,"CANCEL",96,28)then open=false end;ImGui.End(ctx)end;persist();if go then open=false;render();return end;if open then reaper.defer(loop)end end
reaper.defer(loop)
