-- @description 25ch LBAP Dome Panner
-- @author s3g
-- @version 0.9
...
local ctx = ImGui.CreateContext("25CH LBAP DOME PANNER")
local open = true
local PROJECT = 0
local FX_NAME = "s3g 25ch LBAP Dome Panner"
local FX_NAME_CLEAN = "25CH LBAP DOME PANNER"
local FX_NAME_LEGACY = "s3g/25ch LBAP Dome Panner"
...
local function draw_dome(track, fx)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 14, CANVAS.text, "25CH LBAP DOME PANNER")
...
local function loop()
  ...
  local visible
  visible, open = ImGui.Begin(ctx, "25CH LBAP DOME PANNER", open)
  if visible then
