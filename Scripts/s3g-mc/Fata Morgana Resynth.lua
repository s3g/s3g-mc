...
local function combo_table(ctx, label, value, values)
  ...
  row_label(ctx, x, y, label)
  ImGui.SetCursorScreenPos(ctx, control_x, y)
  ImGui.SetNextItemWidth(ctx, combo_table_width(ctx, values, current, control_w))
...
local function main()
  ...
  local function loop()
    ...
    if visible then
      ...
      finish_section(ctx, sx, sy, sh, stack)
      sx, sy, sh, stack = section(ctx, "Render Safety", settings.clarity_protect and (settings.normalize and 226 or 200) or (settings.normalize and 200 or 174))
