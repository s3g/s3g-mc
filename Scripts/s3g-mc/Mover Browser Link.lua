-- @description Mover Browser Link
-- @author s3g
-- @version 0.1
-- @requires ReaImGui; Python 3
-- @category Spatial Panners
-- @method Opens the browser-based s3g-mc Mover with the last loaded Mover JSON and writes a small playhead file so the browser can follow REAPER transport.

local SCRIPT_NAME = "Mover Browser Link"
...
local mover_dir = repo_root .. "/tools/mover"
local docs_mover_dir = repo_root .. "/docs/tools/mover"
...
if not file_exists(mover_dir .. "/index.html") and file_exists(docs_mover_dir .. "/index.html") then
  mover_dir = docs_mover_dir
end
...
local function start_server()
  ...
  if not file_exists(mover_dir .. "/index.html") then
    status = "Could not find tools/mover/index.html."
    return false
  end
  local sep = package.config:sub(1, 1)
  if sep == "\\" then
...
local function choose_json()
  local ok, path = reaper.GetUserFileNameForRead("", "Choose s3g-mc Mover JSON", ".json")
  if ok and path ~= "" then
    json_path = path
...
local function playhead_json()
  ...
  return string.format(
    '{"tool":"s3g-mc Mover Link","version":1,"playing":%s,"position":%.9f,"start":%.9f,"duration":%.9f,"t":%.9f,"updated":%.9f}',
...
local function draw()
  ...
  if visible then
    ImGui.Text(ctx, "s3g-mc Mover browser link")
    ImGui.Separator(ctx)
