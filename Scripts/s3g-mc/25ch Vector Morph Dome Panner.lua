-- @description 25ch Vector Morph Dome Panner
-- @author s3g
-- @version 0.6
...
if not reaper.APIExists("ImGui_GetVersion") then
  reaper.MB("ReaImGui is not installed or not loaded.", "25CH VECTOR MORPH DOME PANNER", 0)
  return
end
...
local ctx = ImGui.CreateContext("25CH VECTOR MORPH DOME PANNER")
local open = true
local PROJECT = 0
local FX_NAME = "s3g 25ch Vector Morph Dome Panner"
local FX_NAME_CLEAN = "25CH VECTOR MORPH DOME PANNER"
local FX_NAME_LEGACY = "s3g/25ch Vector Morph Dome Panner"
local selected_source = 1
local preview_morph = false
...
local function scene_param(scene_index, source_index, offset)
  return 59 + ((scene_index - 1) * 24) + ((source_index - 1) * 3) + offset
end
...
local function lerp_degrees(a, b, t)
...
local function slider_double_changed(track, fx, label, param, lo, hi, fmt, write_enabled)
  local value = get_param(track, fx, param, lo)
  local changed, new_value
  changed, new_value = ui_theme.slider_row(ImGui, ctx, label, value, lo, hi, fmt or "%.2f", false)
  if changed then set_param(track, fx, param, new_value, write_enabled) end
...
local function scene_aed(track, fx, scene, source)
  return
    get_param(track, fx, scene_param(scene, source, 0), (source - 1) * 45),
    get_param(track, fx, scene_param(scene, source, 1), 0),
    get_param(track, fx, scene_param(scene, source, 2), 1)
end

local function morphed_source_aed(track, fx, source)
  local x = get_param(track, fx, PARAM.morph_x, 0)
  local y = get_param(track, fx, PARAM.morph_y, 0)
  local a_az, a_el, a_dist = scene_aed(track, fx, 1, source)
  local b_az, b_el, b_dist = scene_aed(track, fx, 2, source)
  local c_az, c_el, c_dist = scene_aed(track, fx, 3, source)
  local d_az, d_el, d_dist = scene_aed(track, fx, 4, source)
  local ab_az = lerp_degrees(a_az, b_az, x)
  local cd_az = lerp_degrees(c_az, d_az, x)
  local az = lerp_degrees(ab_az, cd_az, y)
  local el = lerp(lerp(a_el, b_el, x), lerp(c_el, d_el, x), y)
  local dist = lerp(lerp(a_dist, b_dist, x), lerp(c_dist, d_dist, x), y)
  return az, el, dist
end

local function live_source_aed(track, fx, source)
  return
    get_param(track, fx, source_param(source, 0), (source - 1) * 45),
    get_param(track, fx, source_param(source, 1), 0),
    get_param(track, fx, source_param(source, 2), 1)
end

local function set_live_source_aed(track, fx, source, az, el, dist)
  set_spatial_param(track, fx, source_param(source, 0), wrap_degrees(az))
  set_spatial_param(track, fx, source_param(source, 1), clamp(el, 0, 90))
  set_spatial_param(track, fx, source_param(source, 2), clamp(dist, 0.1, 3))
end

local function set_scene_source_aed(track, fx, scene, source, az, el, dist)
  set_spatial_param(track, fx, scene_param(scene, source, 0), wrap_degrees(az))
  set_spatial_param(track, fx, scene_param(scene, source, 1), clamp(el, 0, 90))
  set_spatial_param(track, fx, scene_param(scene, source, 2), clamp(dist, 0.1, 3))
end

local function store_live_to_scene(track, fx, scene)
  for source = 1, 8 do
    local az, el, dist = live_source_aed(track, fx, source)
    set_scene_source_aed(track, fx, scene, source, az, el, dist)
  end
end

local function load_scene_to_live(track, fx, scene)
  for source = 1, 8 do
    local az, el, dist = scene_aed(track, fx, scene, source)
    set_live_source_aed(track, fx, source, az, el, dist)
  end
end

local function reset_scene(track, fx, scene)
  local base = (scene - 1) * 90
  for source = 1, 8 do
    set_scene_source_aed(track, fx, scene, source, wrap_degrees(base + (source - 1) * 45), 0, 1)
  end
end

local function set_morph_to_scene(track, fx, scene)
  local x = (scene == 2 or scene == 4) and 1 or 0
  local y = (scene == 3 or scene == 4) and 1 or 0
  set_spatial_param(track, fx, PARAM.morph_x, x)
  set_spatial_param(track, fx, PARAM.morph_y, y)
end

local function load_morphed_to_live(track, fx)
  for source = 1, 8 do
    local az, el, dist = morphed_source_aed(track, fx, source)
    set_live_source_aed(track, fx, source, az, el, dist)
  end
end
...
local function draw_shell_edge(draw_list, by_id, cx, cy)
  ...
  for i = 1, #points do
    local a = points[i]
    local b = points[(i % #points) + 1]
    ImGui.DrawList_AddLine(draw_list, a.x, a.y, b.x, b.y, CANVAS.shell_line, 1.5)
  end
end
...
local function update_source_from_mouse(track, fx, source_index, mx, my, cx, cy, radius, global_az, global_el, global_dist)
  ...
  set_live_source_aed(track, fx, source_index, az - global_az, el - global_el, distance - global_dist)
end
...
local function reset_source_distances(track, fx)
  ...
  for scene = 1, 4 do
    for source = 1, 8 do
      set_spatial_param(track, fx, scene_param(scene, source, 2), 1)
    end
  end
...
local function draw_camera_controls()
  ...
  nudge_camera("L##cam", 32, 24, function()
    view_yaw_deg = view_yaw_deg - 4
  end)
  ImGui.SameLine(ctx)
  nudge_camera("R##cam", 32, 24, function()
    view_yaw_deg = view_yaw_deg + 4
  end)
  nudge_camera("DN##cam", 68, 24, function()
    view_pitch_deg = clamp(view_pitch_deg - 4, -180, 180)
  end)
...
local function draw_dome(track, fx)
  ...
  local sources = {}
  for source = 1, 8 do
    local source_az, source_el, source_dist
    if preview_morph then
      source_az, source_el, source_dist = morphed_source_aed(track, fx, source)
    else
      source_az, source_el, source_dist = live_source_aed(track, fx, source)
    end
    local az = source_az + global_az
    local el = clamp(source_el + global_el, 0, 90)
    local dist = clamp(source_dist + global_dist, 0.1, 3)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 14, CANVAS.text, "25CH VECTOR MORPH DOME PANNER")
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 34, CANVAS.muted, preview_morph and "previewing stored morph" or "editing free live layout")
  ImGui.DrawList_AddText(draw_list, x0 + canvas_width - 330, y0 + 14, CANVAS.muted, preview_morph and "drag copies preview to live" or "drag source dots freely")
...
local function draw_source_controls(track, fx)
  ...
  slider_double(track, fx, base_label .. " live azimuth (deg)", source_param(selected_source, 0), -360, 360, "%.1f", spatial_writes)
  slider_double(track, fx, base_label .. " live elevation (deg)", source_param(selected_source, 1), 0, 90, "%.1f", spatial_writes)
  slider_double(track, fx, base_label .. " live distance (dome radius, edge=1)", source_param(selected_source, 2), 0.1, 3, "%.2f", spatial_writes)
...
local function draw_snapshot_controls(track, fx)
  _, preview_morph = ui_theme.checkbox_row(ImGui, ctx, "Preview morph", preview_morph)
  ImGui.SameLine(ctx)
  if ImGui.Button(ctx, "LOAD MORPHED TO LIVE") then
    load_morphed_to_live(track, fx)
    preview_morph = false
  end

  for scene = 1, 4 do
    local name = SCENES[scene]
    if ImGui.Button(ctx, "STORE " .. name) then
      store_live_to_scene(track, fx, scene)
      set_morph_to_scene(track, fx, scene)
      preview_morph = true
    end
    ImGui.SameLine(ctx)
    if ImGui.Button(ctx, "LOAD " .. name) then
      load_scene_to_live(track, fx, scene)
      preview_morph = false
    end
    ImGui.SameLine(ctx)
    if ImGui.Button(ctx, "CLEAR " .. name) then reset_scene(track, fx, scene) end
  end
end
...
local function loop()
  ...
  local visible
  visible, open = ImGui.Begin(ctx, "25CH VECTOR MORPH DOME PANNER", open)
  if visible then
    ...
    if not track then
      ...
      if fx < 0 then
        ...
        local fx_key = reaper.GetTrackGUID(track) .. ":" .. tostring(fx)
        if initialized_fx_key ~= fx_key then
          load_morphed_to_live(track, fx)
          preview_morph = true
          initialized_fx_key = fx_key
        end
        draw_dome(track, fx)
        ...
        if toolbox_header("GLOBAL", ImGui.TreeNodeFlags_DefaultOpen) then
          local changed_x = slider_double_changed(track, fx, "Morph X", PARAM.morph_x, 0, 1, "%.3f", spatial_writes)
          local changed_y = slider_double_changed(track, fx, "Morph Y", PARAM.morph_y, 0, 1, "%.3f", spatial_writes)
          if changed_x or changed_y then preview_morph = true end
          slider_double(track, fx, "Vector sharpness", PARAM.sharpness, 0.25, 4, "%.2f", spatial_writes)
        ...
        if toolbox_header("SNAPSHOTS", ImGui.TreeNodeFlags_DefaultOpen) then
          draw_snapshot_controls(track, fx)
        end
