...
local function draw_dome(track, fx)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 14, COLORS.text, "25ch Vector Morph Dome Panner")
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 34, COLORS.muted, preview_morph and "previewing stored morph" or "editing free live layout")
  ImGui.DrawList_AddText(draw_list, x0 + canvas_width - 330, y0 + 14, COLORS.muted, preview_morph and "drag copies preview to live" or "drag source dots freely")
  ...
  if visible then
    ...
    if not track then
      ...
      if fx < 0 then
        ...
        if ImGui.CollapsingHeader(ctx, "Snapshots", nil, ImGui.TreeNodeFlags_DefaultOpen) then
