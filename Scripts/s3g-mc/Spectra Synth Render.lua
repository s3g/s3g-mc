-- @description Spectra Synth Render
-- @author s3g
-- @version 0.1
...
-- @method Offline controller for the multichannel Spectra synth engine. Algorithms generate partial clouds, comb strata, formant bands, impulse resonators, and noise spectra with algorithm-specific channel-motion models. Choose duration, algorithm, and map-route breakpoint curves; the action writes automation to a temporary generator track, renders the selected range as a multichannel stem, then removes the temporary track.
...
if not reaper.APIExists("ImGui_GetVersion") then
  reaper.MB("ReaImGui is not installed or not loaded.", "Spectra Synth Render", 0)
  return
end
...
local function render_texture(settings)
  ...
  mc.with_ui_refresh_block(function()
    ...
    local synth_track = mc.insert_track_at(insert_index, "tmp Spectra Synth", mc.reaper_track_channel_count(channels))
      ...
      if did_render then
        ...
        if render_track then
          reaper.GetSetMediaTrackInfo_String(render_track, "P_NAME",
            "Spectra Synth render (" .. tostring(channels) .. "ch)", true)
  ...
  if selected_def and selected_points then
    ...
    for index, point in ipairs(selected_points) do
      ...
      draw_square_handle(draw_list, px, py, index == selected_point and 10.5 or 8.0,
        index == selected_point and STYLE.point_selected or STYLE.point, STYLE.edge, 1.4)
    end
...
local function draw_route_editor(ctx, points, def, selected_index, enabled)
  ...
  for index, point in ipairs(points) do
    ...
    draw_square_handle(draw_list, px, py, index == selected_index and 10.5 or 8.0,
      index == selected_index and STYLE.point_selected or STYLE.point)
  end
    ...
    for point_index, point in ipairs(points) do
      ...
      if enabled or is_selected then
        draw_square_handle(draw_list, px, py, is_selected and point_index == selected_point and 8.5 or 6.5,
          is_selected and point_index == selected_point and STYLE.point_selected or STYLE.point)
      end
      last_x, last_y = px, py
    end
...
ctx = ImGui.CreateContext("Spectra Synth Render")
...
local function loop()
  ...
  local visible
  visible, open = ImGui.Begin(ctx, "Spectra Synth Render", open)
  if visible then
    local changed
    ...
    if ImGui.BeginChild(ctx, "##spectra_controls", 0, control_h) then
    ...
    theme.muted(ImGui, ctx, string.format("END %.3f SEC", math.max(0, start_pos) + duration))
