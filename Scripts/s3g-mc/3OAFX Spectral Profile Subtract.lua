...
local function draw_mask_icon(draw_list, x0, y0, x1, y1)
  ImGui.DrawList_AddRectFilled(draw_list, x0, y0, x1, y1, FLOW.panel)
  ImGui.DrawList_AddRect(draw_list, x0, y0, x1, y1, FLOW.profile)
  ImGui.DrawList_AddText(draw_list, x0 + 9, y0 + 9, FLOW.text, "spectral profile")
  ImGui.DrawList_AddText(draw_list, x0 + 9, y0 + 30, FLOW.muted, "per-direction mask")
end
...
local function draw_flow(ctx, settings)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 12, FLOW.text, "directional spectral profile subtraction")
  ...
  draw_box(draw_list, left_x, profile_y, left_x + left_w, profile_y + box_h, "profile HOA", "material to remove", FLOW.profile)
  ...
  draw_box(draw_list, proc_x, y0 + 83, proc_x + proc_w, y0 + 83 + box_h, "subtract / gate", "same direction bins", FLOW.flow)
  draw_box(draw_list, out_x, y0 + 83, out_x + out_w, y0 + 83 + box_h, OUTPUT_NAMES[settings.output_index], "re-encoded HOA", FLOW.output)
  ...
  ImGui.DrawList_AddText(draw_list, left_x + 4, y1 - 24, FLOW.muted, "Profile audio is analyzed into a spectral mask; it is not mixed into the source path.")
end
