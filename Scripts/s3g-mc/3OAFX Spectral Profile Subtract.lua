...
local TITLE = "3OAFX Spectral Profile Subtract"
...
local OUTPUT_NAMES = { "Cleaned source", "Residue only" }
local OUTPUT_KEYS = { "cleaned", "residue" }
...
local function validate(source, profile, settings)
  if not source or not profile then return "Select a source item and a profile item." end
  ...
  if profile.filename == "" or not nr.file_exists(profile.filename) or not is_wav(profile.filename) then
    return "The profile item must be backed by a readable WAV file."
  end
  ...
  if profile.channels < needed then
    return "The profile item has " .. tostring(profile.channels) .. " channels, but this order needs " .. tostring(needed) .. "."
  end
  return nil
end
...
local function draw_mask_icon(draw_list, x0, y0, x1, y1)
  ImGui.DrawList_AddRectFilled(draw_list, x0, y0, x1, y1, COLOR_PANEL)
  ImGui.DrawList_AddRect(draw_list, x0, y0, x1, y1, COLOR_PROFILE)
  ImGui.DrawList_AddText(draw_list, x0 + 9, y0 + 9, COLOR_TEXT, "spectral profile")
  ImGui.DrawList_AddText(draw_list, x0 + 9, y0 + 30, COLOR_MUTED, "per-direction mask")
end

local function draw_flow(ctx, settings)
  local width = math.max(560, ImGui.GetContentRegionAvail(ctx) - 2)
  local height = 214
  ImGui.InvisibleButton(ctx, "##profile_subtract_flow", width, height)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 12, COLOR_TEXT, "directional spectral profile subtraction")
  ...
  local out_w = width - margin * 2 - gap * 3 - left_w - mid_w - proc_w
  out_w = math.max(130, out_w)
  ...
  draw_box(draw_list, left_x, profile_y, left_x + left_w, profile_y + box_h, "profile HOA", "material to remove", COLOR_PROFILE)
  ...
  draw_mask_icon(draw_list, mid_x, profile_y, mid_x + mid_w, profile_y + box_h)
  draw_box(draw_list, proc_x, y0 + 83, proc_x + proc_w, y0 + 83 + box_h, "subtract / gate", "same direction bins", COLOR_FLOW)
  draw_box(draw_list, out_x, y0 + 83, out_x + out_w, y0 + 83 + box_h, OUTPUT_NAMES[settings.output_index], "re-encoded HOA", COLOR_OUTPUT)
  ...
  ImGui.DrawList_AddText(draw_list, left_x + 4, y1 - 24, COLOR_MUTED, "Profile audio is analyzed into a spectral mask; it is not mixed into the source path.")
end
...
local function run_render(source, profile, settings)
  ...
  local output_dir = nr.output_dir("s3g_foafx_profile_subtract_renders", source.filename, script_dir)
  local output_path = output_dir .. "/s3g_foafx_profile_subtract_" .. stamp .. "_" .. tostring(order) .. "oa.wav"
  ...
  local manifest = {
    ...
    order = order,
    output_mode = OUTPUT_KEYS[settings.output_index] or "cleaned",
  ...
  local log, elapsed = nr.run_backend(script_dir, "foafx_profile_subtract", manifest, TITLE)
  if not log then return end

  reaper.Undo_BeginBlock()
  local item, insert_err = nr.insert_output_item(output_path, "3OAFX profile subtract (" .. tostring(order) .. "OA)", source.position, output_channels, {
    master_send = false,
    track_gain = 0.5,
  })
  ...
  local lines = {
    ...
    "Profile: " .. profile.name .. " (" .. tostring(profile.channels) .. "ch)",
...
local function main()
  ...
  if #entries < 2 then
    mc.show_error("Select two WAV-backed ambisonic media items. The earliest selected item is the source; the next selected item is the profile to remove.")
    return
  end
  ...
  local settings = {
    ...
    output_index = clamp(math.floor(get_number("output_index", 1)), 1, #OUTPUT_NAMES),
    profile_index = clamp(math.floor(get_number("profile_index", 1)), 1, #PROFILE_NAMES),
    reduction_amount = get_number("reduction_amount", 0.72),
    spectral_floor = get_number("spectral_floor", 0.18),
    profile_sensitivity = get_number("profile_sensitivity", 1.15),
    frequency_smoothing_bins = get_number("frequency_smoothing_bins", 3),
    temporal_smoothing = get_number("temporal_smoothing", 0.35),
    fft_index = clamp(math.floor(get_number("fft_index", 2)), 1, #FFT_NAMES),
    overlap = clamp(math.floor(get_number("overlap", 4)), 2, 8),
  ...
  local function loop()
    ...
    if visible then
      ...
      ImGui.Text(ctx, "Profile: " .. profile.name .. "  (" .. tostring(profile.channels) .. " ch)")
      ImGui.Spacing(ctx)
      ...
      local changed
      changed, settings.reduction_amount = ImGui.SliderDouble(ctx, "Reduction amount", settings.reduction_amount, 0.0, 1.0, "%.2f")
      changed, settings.spectral_floor = ImGui.SliderDouble(ctx, "Spectral floor", settings.spectral_floor, 0.0, 0.75, "%.2f")
      changed, settings.profile_sensitivity = ImGui.SliderDouble(ctx, "Profile sensitivity", settings.profile_sensitivity, 0.25, 4.0, "%.2f")
      ...
      ImGui.Text(ctx, "Profile file: " .. basename(profile.filename))
      if validation then
