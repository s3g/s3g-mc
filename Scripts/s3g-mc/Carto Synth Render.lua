...
local function write_amplitude_curve_to_gmem(routes, duration)
  ...
  reaper.gmem_attach(CARTO_GMEM_NAME)
...
local function draw_carto_map(ctx, route_points, route_enabled, selected_route, selected_point, channels, values, algorithm)
  local width = ImGui.GetContentRegionAvail(ctx)
  local height = 178
  local draw_list = ImGui.GetWindowDrawList(ctx)
  local x0, y0 = ImGui.GetCursorScreenPos(ctx)
  local x1, y1 = x0 + width, y0 + height
  ImGui.InvisibleButton(ctx, "##carto_map", width, height)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 12, y0 + 8, COLORS.text, "Carto map")
...
for index, def in ipairs(ROUTE_DEFS) do
  route_enabled[index] = false
  route_points[index] = {}
  set_flat_route(route_points[index], def, current_value_for_key(carto_values(), def.key))
end
