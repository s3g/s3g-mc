...
local function reset_source_distances(track, fx)
  for source = 1, 8 do set_spatial_param(track, fx, source_param(source, 2), 1) end
  set_spatial_param(track, fx, PARAM.global_dist, 0)
end

local function clear_mutes(track, fx)
  for source = 1, 8 do set_mix_param(track, fx, source_control_param(source, 1), 0) end
end

local function clear_solos(track, fx)
  for source = 1, 8 do set_mix_param(track, fx, source_control_param(source, 2), 0) end
end
...
local function draw_camera_controls()
  ...
  nudge_camera("L##cam", 32, 24, function() view_yaw_deg = view_yaw_deg - 4 end)
  ImGui.SameLine(ctx)
  nudge_camera("R##cam", 32, 24, function() view_yaw_deg = view_yaw_deg + 4 end)
  nudge_camera("DN##cam", 68, 24, function() view_pitch_deg = clamp(view_pitch_deg - 4, -180, 180) end)
...
local function draw_layout(track, fx)
  ...
  if canvas_hovered or canvas_active or dragging_source > 0 then
    ...
    if canvas_hovered and ImGui.IsMouseClicked(ctx, 0) then
      ...
      if hit > 0 then selected_source = hit if spatial_writes then dragging_source = hit end end
    end
...
local function layout_combo(track, fx)
  ...
  for index, layout in ipairs(LAYOUTS) do
    labels[index] = layout.name .. " (" .. tostring(layout.channels) .. "CH)"
  end
  local changed, next_index = ui_theme.combo_row(ImGui, ctx, "Layout", labels, current)
  if changed then
    current = next_index
    local layout = LAYOUTS[current]
    set_spatial_param(track, fx, PARAM.layout, current - 1)
...
local function draw_source_controls(track, fx)
  ...
  slider_double(track, fx, base_label .. " distance (radius, edge=1)", source_param(selected_source, 2), 0.1, 3, "%.2f", spatial_writes)
