-- @description 25ch VBAP Dome Panner
-- @author s3g
-- @version 0.9
-- @requires ReaImGui; JSFX: s3g 25ch VBAP Dome Panner
-- @category Spatial Panners
-- @method Auto-loads the JSFX on the selected track and pans up to 8 mono source channels across the 25-speaker dome layout of the RISD SRST Spatial Audio Studio using VBAP-inspired nearest-region weighting.
-- @about
--   ReaImGui companion controller for JS: s3g 25ch VBAP Dome Panner.
...
if not reaper.APIExists("ImGui_GetVersion") then
  reaper.MB("ReaImGui is not installed or not loaded.", "25CH VBAP DOME PANNER", 0)
  return
end
...
local ctx = ImGui.CreateContext("25CH VBAP DOME PANNER")
local open = true
local PROJECT = 0
local FX_NAME = "s3g 25ch VBAP Dome Panner"
local FX_NAME_CLEAN = "25CH VBAP DOME PANNER"
local FX_NAME_LEGACY = "s3g/25ch VBAP Dome Panner"
...
local function draw_dome(track, fx)
  ...
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 14, CANVAS.text, "25CH VBAP DOME PANNER")
  ImGui.DrawList_AddText(draw_list, x0 + 14, y0 + 34, CANVAS.muted, "25 speakers / 8 mono sources / distance 1.0 = dome edge")
  ImGui.DrawList_AddText(draw_list, x0 + canvas_width - 300, y0 + 14, CANVAS.muted,
