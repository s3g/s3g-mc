...
function chooseInitialGradientScore() {
  const gradientRules = ["gradient_linear", "gradient_radial", "gradient_bands", "gradient_vortex"];
  const palettes = ["aed_full", "front_focus", "over_under", "dusk", "ember_ice", "terrain", "signal"];
  const rand = Math.random;
  controls.rule.value = gradientRules[Math.floor(rand() * gradientRules.length)];
  controls.palette.value = palettes[Math.floor(rand() * palettes.length)];
  controls.seed.value = String(1000 + Math.floor(rand() * 900000));
  controls.density.value = String(34 + Math.floor(rand() * 48));
  controls.freqSpread.value = String(45 + Math.floor(rand() * 46));
  controls.gestureDrift.value = String(22 + Math.floor(rand() * 62));
}
