...
local function loop()ImGui.SetNextWindowSize(ctx,520,430,ImGui.Cond_Appearing);local vis;vis,open=ImGui.Begin(ctx,"Subharmonic Bank",open);if vis then local changed;changed,s.duration=ImGui.SliderDouble(ctx,"Duration sec",s.duration,0.5,180,"%.2f");changed,s.channels=ImGui.SliderInt(ctx,"Channels",math.floor(s.channels),1,mc.MAX_REAPER_TRACK_CHANNELS);changed,s.voices=ImGui.SliderInt(ctx,"Voices",math.floor(s.voices),1,96);changed,s.root_freq=ImGui.SliderDouble(ctx,"Root frequency",s.root_freq,20,880,"%.1f");changed,s.instability=ImGui.SliderDouble(ctx,"Instability",s.instability,0,1,"%.2f");changed,s.pulse_blend=ImGui.SliderDouble(ctx,"Pulse blend",s.pulse_blend,0,1,"%.2f");changed,s.fold=ImGui.SliderDouble(ctx,"Fold / drive",s.fold,0,1,"%.2f");changed,s.event_mask=ImGui.SliderDouble(ctx,"Mask openings",s.event_mask,0,1,"%.2f");changed,s.spatial_width=ImGui.SliderDouble(ctx,"Spatial width",s.spatial_width,0.2,8,"%.2f");changed,s.normalize=ImGui.Checkbox(ctx,"Peak normalize",s.normalize);if s.normalize then changed,s.normalize_db=ImGui.SliderDouble(ctx,"Normalize dB",s.normalize_db,-36,0,"%.1f")end;changed,s.seed=ImGui.InputInt(ctx,"Seed",math.floor(s.seed));if ImGui.Button(ctx,"Render",96,28)then go=true end;ImGui.SameLine(ctx);if ImGui.Button(ctx,"Cancel",96,28)then open=false end;ImGui.End(ctx)end;persist();if go then open=false;render();return end;if open then reaper.defer(loop)end end
reaper.defer(loop)
