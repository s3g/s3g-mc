...
local function section(label,height)local stack=theme.push_soft_panel(ImGui,ctx);local draw=ImGui.GetWindowDrawList(ctx);local x,y=ImGui.GetCursorScreenPos(ctx);local w=ImGui.GetContentRegionAvail(ctx);ImGui.DrawList_AddRectFilled(draw,x,y,x+w,y+height,THEME.panel_soft);ImGui.DrawList_AddRectFilled(draw,x,y,x+w,y+2,THEME.active);ImGui.SetCursorScreenPos(ctx,x+12,y+10);theme.text(ImGui,ctx,label:upper());ImGui.SetCursorScreenPos(ctx,x+12,y+36);return x,y,height,stack end
