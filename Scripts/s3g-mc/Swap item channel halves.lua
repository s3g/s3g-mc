-- @description Swap item channel halves
-- @author s3g
-- @version 0.1
...
-- @method Renders a copy of the selected multichannel item with the lower and upper channel halves swapped.
-- @about
--   Converts 1 2 3 4 5 6 7 8 to 5 6 7 8 1 2 3 4.
...
local function main()
  ...
  local channel_map = mc.swap_halves_map(channel_count)

  reaper.Undo_BeginBlock()
  local did_render = mc.with_ui_refresh_block(function()
    return mc.build_multichannel_render_from_item(item, channel_count,
      channel_map, "Swapped channel halves", { mute_source_item = MUTE_SOURCE_ITEM_AFTER_RENDER })
  end)
  reaper.Undo_EndBlock("Swap channel halves of selected item", -1)

  if did_render then
    mc.print_plan("Swapped item channel halves", mc.render_plan_for_item(item, channel_count,
      channel_map, "Swapped channel halves"))
