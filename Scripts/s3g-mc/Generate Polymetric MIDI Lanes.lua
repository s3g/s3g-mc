...
local function loop()
  ...
  if visible then
    ...
    changed, root_index = ImGui.Combo(ctx, "Root", root_index - 1, table.concat(midi.ROOT_NAMES, "\0") .. "\0")
    root_index = root_index + 1
    ImGui.SameLine(ctx)
    ImGui.SetNextItemWidth(ctx, 170)
    changed, scale_index = ImGui.Combo(ctx, "Scale", scale_index - 1, table.concat(midi.SCALE_NAMES, "\0") .. "\0")
